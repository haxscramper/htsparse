...
func `[]`*(
    node: TsCsharpNode,
    idx:  int,
    kind: CsharpNodeKind | set[CsharpNodeKind]
  ): TsCsharpNode =
  assert 0 <= idx and idx < node.len
  result = TsCsharpNode(ts_node_named_child(TSNode(node), uint32(idx)))
  assertKind(result, kind, "Child node at index " & $idx & " for node kind " & $node.kind)

type
  CsharpNode* = HtsNode[TsCsharpNode, CsharpNodeKind]


proc treeReprTsCsharp*(str: string, unnamed: bool = false): ColoredText =
  treeRepr[TsCsharpNode, CsharpNodeKind](parseTsCsharpString(str), str, 6, unnamed = unnamed)

proc toHtsNode*(
    node: TsCsharpNode,
    str:  ptr string
  ): HtsNode[TsCsharpNode, CsharpNodeKind] =
  toHtsNode[TsCsharpNode, CsharpNodeKind](node, str)

proc toHtsTree*(node: TsCsharpNode, str: ptr string): CsharpNode =
  toHtsNode[TsCsharpNode, CsharpNodeKind](node, str)

proc parseCsharpString*(str: ptr string, unnamed: bool = false): CsharpNode =
  let parser = newTsCsharpParser()
  return toHtsTree[TsCsharpNode, CsharpNodeKind](parseString(parser, str[]), str)

proc parseCsharpString*(str: string, unnamed: bool = false): CsharpNode =
  let parser = newTsCsharpParser()
  return toHtsTree[TsCsharpNode, CsharpNodeKind](parseString(parser, str), unsafeAddr str, storePtr = false)
