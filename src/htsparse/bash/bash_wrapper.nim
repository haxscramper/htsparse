...
func `[]`*(
    node: TsBashNode,
    idx:  int,
    kind: BashNodeKind | set[BashNodeKind]
  ): TsBashNode = 
...
proc treeReprTsBash*(str: string, unnamed: bool = false): ColoredText = 
  treeRepr[TsBashNode, BashNodeKind](parseTsBashString(str), str, 4, unnamed = unnamed)
 
proc toHtsNode*(
    node: TsBashNode,
    str:  ptr string
  ): HtsNode[TsBashNode, BashNodeKind] = 
...
proc parseBashString*(str: string, unnamed: bool = false): BashNode = 
  let parser = newTsBashParser()
  return toHtsTree[TsBashNode, BashNodeKind](parseString(parser, str), unsafeAddr str, storePtr = false)
