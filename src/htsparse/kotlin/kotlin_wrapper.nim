...
type
  KotlinNodeKind* = enum
    ...
    kotlinMultiVariableDeclaration     ## multi_variable_declaration
...
type
  KotlinExternalTok* = enum
    kotlinExtern_automatic_semicolon ## _automatic_semicolon
...
proc kind*(node: TsKotlinNode): KotlinNodeKind {.noSideEffect.} =
  {.cast(noSideEffect).}:
    case node.tsNodeType:
      ...
      of "multi_variable_declaration":     kotlinMultiVariableDeclaration
...
func `[]`*(
    node: TsKotlinNode,
    idx:  int,
    kind: KotlinNodeKind | set[KotlinNodeKind]
  ): TsKotlinNode =
  assert 0 <= idx and idx < node.len
  result = TsKotlinNode(ts_node_named_child(TSNode(node), uint32(idx)))
  assertKind(result, kind, "Child node at index " & $idx & " for node kind " & $node.kind)

type
  KotlinNode* = HtsNode[TsKotlinNode, KotlinNodeKind]

proc treeReprTsKotlin*(str: string, unnamed: bool = false): ColoredText =
  treeRepr[TsKotlinNode, KotlinNodeKind](parseTsKotlinString(str), str, 6, unnamed = unnamed)

proc toHtsNode*(
    node: TsKotlinNode,
    str:  ptr string
  ): HtsNode[TsKotlinNode, KotlinNodeKind] =
  toHtsNode[TsKotlinNode, KotlinNodeKind](node, str)
...
proc parseKotlinString*(str: ptr string, unnamed: bool = false): KotlinNode =
  let parser = newTsKotlinParser()
  return toHtsTree[TsKotlinNode, KotlinNodeKind](parseString(parser, str[]), str)

proc parseKotlinString*(str: string, unnamed: bool = false): KotlinNode =
  let parser = newTsKotlinParser()
  return toHtsTree[TsKotlinNode, KotlinNodeKind](parseString(parser, str), unsafeAddr str, storePtr = false)
