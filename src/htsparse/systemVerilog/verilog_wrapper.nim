...
func `[]`*(
    node: TsVerilogNode,
    idx:  int,
    kind: VerilogNodeKind | set[VerilogNodeKind]
  ): TsVerilogNode =
...
proc treeReprTsVerilog*(str: string, unnamed: bool = false): ColoredText =
  treeRepr[TsVerilogNode, VerilogNodeKind](parseTsVerilogString(str), str, 7, unnamed = unnamed)

proc toHtsNode*(
    node: TsVerilogNode,
    str:  ptr string
  ): HtsNode[TsVerilogNode, VerilogNodeKind] =
...
proc parseVerilogString*(str: ptr string, unnamed: bool = false): VerilogNode =
  let parser = newTsVerilogParser()
  return toHtsTree[TsVerilogNode, VerilogNodeKind](parseString(parser, str[]), str)

proc parseVerilogString*(str: string, unnamed: bool = false): VerilogNode =
  let parser = newTsVerilogParser()
  return toHtsTree[TsVerilogNode, VerilogNodeKind](parseString(parser, str), unsafeAddr str, storePtr = false)
