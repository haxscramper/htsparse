...
type
  VerilogNodeKind* = enum
    verilogSingleDollarfulskewSingleUnderscoretimingSingleUnderscorecheck, ## $fullskew_timing_check
    verilogSingleDollarholdSingleUnderscoretimingSingleUnderscorecheck, ## $hold_timing_check
    verilogSingleDollarnochangeSingleUnderscoretimingSingleUnderscorecheck, ## $nochange_timing_check
    verilogSingleDollarperiodSingleUnderscoretimingSingleUnderscorecheck, ## $period_timing_check
    verilogSingleDollarrecoverySingleUnderscoretimingSingleUnderscorecheck, ## $recovery_timing_check
    verilogSingleDollarrecremSingleUnderscoretimingSingleUnderscorecheck, ## $recrem_timing_check
    verilogSingleDollarremovalSingleUnderscoretimingSingleUnderscorecheck, ## $removal_timing_check
    verilogSingleDollarsetupSingleUnderscoretimingSingleUnderscorecheck, ## $setup_timing_check
    verilogSingleDollarsetupholdSingleUnderscoretimingSingleUnderscorecheck, ## $setuphold_timing_check
    verilogSingleDollarskewSingleUnderscoretimingSingleUnderscorecheck, ## $skew_timing_check
    verilogSingleDollartimeskewSingleUnderscoretimingSingleUnderscorecheck, ## $timeskew_timing_check
    verilogSingleDollarwidthSingleUnderscoretimingSingleUnderscorecheck, ## $width_timing_check
    ...
    verilogSingleNewlineTok, ## 
                              ## 
    verilogSingleExclamationTok, ## !
    verilogSingleExclamationSingleEqualTok, ## !=
    verilogSingleExclamationDoubleEqualTok, ## !==
    verilogSingleExclamationSingleEqualSingleQuestionTok, ## !=?
    verilogSingleQuoteTok,  ## "
    verilogSingleQuoteDPISingleQuoteTok, ## "DPI"
    verilogSingleQuoteDPISingleMinusCSingleQuoteTok, ## "DPI-C"
    verilogSingleHashTok,   ## #
    verilogDoubleHashTok,   ## ##
    verilogDoubleHashSingleLBrackSingleAsteriskSingleRBrackTok, ## ##[*]
    verilogDoubleHashSingleLBrackSinglePlusSingleRBrackTok, ## ##[+]
    verilogSingleHashSingleMinusSingleHashTok, ## #-#
    verilogSingleHash0Tok,  ## #0
    verilogSingleHashSingleEqualSingleHashTok, ## #=#
    verilogSingleDollarTok, ## $
    verilogSingleDollarerorTok, ## $error
    verilogSingleDollarfatalTok, ## $fatal
    verilogSingleDollarfulskewTok, ## $fullskew
    verilogSingleDollarholdTok, ## $hold
    verilogSingleDollarinfoTok, ## $info
    verilogSingleDollarnochangeTok, ## $nochange
    verilogSingleDollarperiodTok, ## $period
    verilogSingleDollarrecoveryTok, ## $recovery
    verilogSingleDollarrecremTok, ## $recrem
    verilogSingleDollarremovalTok, ## $removal
    verilogSingleDollarrotTok, ## $root
    verilogSingleDollarsetupTok, ## $setup
    verilogSingleDollarsetupholdTok, ## $setuphold
    verilogSingleDollarskewTok, ## $skew
    verilogSingleDollartimeskewTok, ## $timeskew
    verilogSingleDollarunitTok, ## $unit
    verilogSingleDollarwarningTok, ## $warning
    verilogSingleDollarwidthTok, ## $width
    verilogSinglePercentTok, ## %
    verilogSinglePercentSingleEqualTok, ## %=
    verilogSingleAmpersandTok, ## &
    verilogDoubleAmpersandTok, ## &&
    verilogTripleAmpersandTok, ## &&&
    verilogSingleAmpersandSingleEqualTok, ## &=
    verilogSingleApostropheTok, ## '
    verilogSingleApostrophe0Tok, ## '0
    verilogSingleApostrophe1Tok, ## '1
    verilogSingleApostropheB0Tok, ## 'B0
    verilogSingleApostropheB1Tok, ## 'B1
    verilogSingleApostropheb0Tok, ## 'b0
    verilogSingleApostropheb1Tok, ## 'b1
    verilogSingleApostropheSingleLCurlyTok, ## '{
    verilogSingleLParTok,   ## (
    verilogSingleLParSingleAsteriskTok, ## (*
    verilogSingleRParTok,   ## )
    verilogSingleAsteriskTok, ## *
    verilogSingleAsteriskSingleRParTok, ## *)
    verilogDoubleAsteriskTok, ## **
    verilogSingleAsteriskDoubleColonSingleAsteriskTok, ## *::*
    verilogSingleAsteriskSingleEqualTok, ## *=
    verilogSingleAsteriskSingleGreaterThanTok, ## *>
    verilogSinglePlusTok,   ## +
    verilogDoublePlusTok,   ## ++
    verilogSinglePlusSingleColonTok, ## +:
    verilogSinglePlusSingleEqualTok, ## +=
    verilogSingleCommaTok,  ## ,
    verilogSingleMinusTok,  ## -
    verilogDoubleMinusTok,  ## --
    verilogSingleMinusSingleColonTok, ## -:
    verilogSingleMinusSingleEqualTok, ## -=
    verilogSingleMinusSingleGreaterThanTok, ## ->
    verilogSingleMinusDoubleGreaterThanTok, ## ->>
    verilogSingleDotTok,    ## .
    verilogSingleDotSingleAsteriskTok, ## .*
    verilogSingleSlashTok,  ## /
    verilogSingleSlashSingleEqualTok, ## /=
    verilog0Tok,            ## 0
    verilog01Tok,           ## 01
    verilog1Tok,            ## 1
    verilog1SingleApostropheB0Tok, ## 1'B0
    verilog1SingleApostropheB1Tok, ## 1'B1
    verilog1SingleApostropheBXTok, ## 1'BX
    verilog1SingleApostropheBxTok, ## 1'Bx
    verilog1SingleApostropheb0Tok, ## 1'b0
    verilog1SingleApostropheb1Tok, ## 1'b1
    verilog1SingleApostrophebXTok, ## 1'bX
    ...
    verilog2Tok,            ## 2
    verilogSingleColonTok,  ## :
    verilogSingleColonSingleSlashTok, ## :/
    verilogDoubleColonTok,  ## ::
    verilogSingleColonSingleEqualTok, ## :=
    verilogSingleSemicolonTok, ## ;
    verilogSingleLessThanTok, ## <
    verilogSingleLessThanSingleMinusSingleGreaterThanTok, ## <->
    verilogDoubleLessThanTok, ## <<
    verilogTripleLessThanTok, ## <<<
    verilogTripleLessThanSingleEqualTok, ## <<<=
    verilogDoubleLessThanSingleEqualTok, ## <<=
    verilogSingleLessThanSingleEqualTok, ## <=
    verilogSingleEqualTok,  ## =
    verilogDoubleEqualTok,  ## ==
    verilogTripleEqualTok,  ## ===
    verilogDoubleEqualSingleQuestionTok, ## ==?
    verilogSingleEqualSingleGreaterThanTok, ## =>
    verilogSingleGreaterThanTok, ## >
    verilogSingleGreaterThanSingleEqualTok, ## >=
    verilogDoubleGreaterThanTok, ## >>
    verilogDoubleGreaterThanSingleEqualTok, ## >>=
    verilogTripleGreaterThanTok, ## >>>
    verilogTripleGreaterThanSingleEqualTok, ## >>>=
    verilogSingleQuestionTok, ## ?
    verilogSingleAtTok,     ## @
    verilogSingleAtSingleAsteriskTok, ## @*
    verilogDoubleAtTok,     ## @@
    verilogPATHPULSESingleDollarSingleEqualTok, ## PATHPULSE$=
    verilogSingleLBrackTok, ## [
    verilogSingleLBrackSingleAsteriskTok, ## [*
    verilogSingleLBrackSingleAsteriskSingleRBrackTok, ## [*]
    verilogSingleLBrackSinglePlusSingleRBrackTok, ## [+]
    verilogSingleLBrackSingleMinusSingleGreaterThanTok, ## [->
    verilogSingleLBrackSingleEqualTok, ## [=
    verilogSingleLBrackSingleâSingle€Single“SingleGreaterThanTok, ## [â€“>
    verilogSingleBackslashTok, ## \
    verilogSingleRBrackTok, ## ]
    verilogSingleAccentTok, ## ^
    verilogSingleAccentSingleEqualTok, ## ^=
    verilogSingleAccentSingleTildeTok, ## ^~
    verilogSingleBacktickTok, ## `
    ...
    verilogVoidTok,         ## void
    verilogVoidSingleApostropheTok, ## void'
    verilogWaitTok,         ## wait
    ...
    verilogXnorTok,         ## xnor
    verilogXorTok,          ## xor
    verilogSingleLCurlyTok, ## {
    verilogSinglePipeTok,   ## |
    verilogSinglePipeSingleMinusSingleGreaterThanTok, ## |->
    verilogSinglePipeSingleEqualTok, ## |=
    verilogSinglePipeSingleEqualSingleGreaterThanTok, ## |=>
    verilogDoublePipeTok,   ## ||
    verilogSingleRCurlyTok, ## }
    verilogSingleTildeTok,  ## ~
    verilogSingleTildeSingleAmpersandTok, ## ~&
    verilogSingleTildeSingleAccentTok, ## ~^
    verilogSingleTildeSinglePipeTok, ## ~|
    verilogSingleâSingle€Single“Tok, ## â€“
    verilogSingleâSingle€Single“SingleGreaterThanTok, ## â€“>
    verilogSingleâSingle€Single“SingleâSingle€Single“Tok, ## â€“â€“
    verilogSyntaxError       ## Tree-sitter parser syntax error
type
  VerilogExternalTok* = enum
type
  VerilogNode* = distinct TSNode
type
  VerilogParser* = distinct PtsParser
proc tsNodeType*(node: VerilogNode): string
proc kind*(node: VerilogNode): VerilogNodeKind =
  case node.tsNodeType
  of "$fullskew_timing_check":
    verilogSingleDollarfulskewSingleUnderscoretimingSingleUnderscorecheck
  of "$hold_timing_check":
    verilogSingleDollarholdSingleUnderscoretimingSingleUnderscorecheck
  of "$nochange_timing_check":
    verilogSingleDollarnochangeSingleUnderscoretimingSingleUnderscorecheck
  of "$period_timing_check":
    verilogSingleDollarperiodSingleUnderscoretimingSingleUnderscorecheck
  of "$recovery_timing_check":
    verilogSingleDollarrecoverySingleUnderscoretimingSingleUnderscorecheck
  of "$recrem_timing_check":
    verilogSingleDollarrecremSingleUnderscoretimingSingleUnderscorecheck
  of "$removal_timing_check":
    verilogSingleDollarremovalSingleUnderscoretimingSingleUnderscorecheck
  of "$setup_timing_check":
    verilogSingleDollarsetupSingleUnderscoretimingSingleUnderscorecheck
  of "$setuphold_timing_check":
    verilogSingleDollarsetupholdSingleUnderscoretimingSingleUnderscorecheck
  of "$skew_timing_check":
    verilogSingleDollarskewSingleUnderscoretimingSingleUnderscorecheck
  of "$timeskew_timing_check":
    verilogSingleDollartimeskewSingleUnderscoretimingSingleUnderscorecheck
  of "$width_timing_check":
    verilogSingleDollarwidthSingleUnderscoretimingSingleUnderscorecheck
  ...
  of "\"":
    verilogSingleQuoteTok
  of "\"DPI\"":
    verilogSingleQuoteDPISingleQuoteTok
  of "\"DPI-C\"":
    verilogSingleQuoteDPISingleMinusCSingleQuoteTok
  of "#":
    verilogSingleHashTok
  of "##":
    verilogDoubleHashTok
  ...
  of "#-#":
    verilogSingleHashSingleMinusSingleHashTok
  of "#0":
    verilogSingleHash0Tok
  of "#=#":
    verilogSingleHashSingleEqualSingleHashTok
  of "$":
    verilogSingleDollarTok
  of "$error":
    verilogSingleDollarerorTok
  of "$fatal":
    verilogSingleDollarfatalTok
  of "$fullskew":
    verilogSingleDollarfulskewTok
  of "$hold":
    verilogSingleDollarholdTok
  of "$info":
    verilogSingleDollarinfoTok
  of "$nochange":
    verilogSingleDollarnochangeTok
  of "$period":
    verilogSingleDollarperiodTok
  of "$recovery":
    verilogSingleDollarrecoveryTok
  of "$recrem":
    verilogSingleDollarrecremTok
  of "$removal":
    verilogSingleDollarremovalTok
  of "$root":
    verilogSingleDollarrotTok
  of "$setup":
    verilogSingleDollarsetupTok
  of "$setuphold":
    verilogSingleDollarsetupholdTok
  of "$skew":
    verilogSingleDollarskewTok
  of "$timeskew":
    verilogSingleDollartimeskewTok
  of "$unit":
    verilogSingleDollarunitTok
  of "$warning":
    verilogSingleDollarwarningTok
  of "$width":
    verilogSingleDollarwidthTok
  of "%":
    verilogSinglePercentTok
  of "%=":
    verilogSinglePercentSingleEqualTok
  ...
  of "\'":
    verilogSingleApostropheTok
  of "\'0":
    verilogSingleApostrophe0Tok
  of "\'1":
    verilogSingleApostrophe1Tok
  of "\'B0":
    verilogSingleApostropheB0Tok
  of "\'B1":
    verilogSingleApostropheB1Tok
  of "\'b0":
    verilogSingleApostropheb0Tok
  of "\'b1":
    verilogSingleApostropheb1Tok
  ...
  of "1":
    verilog1Tok
  of "1\'B0":
    verilog1SingleApostropheB0Tok
  of "1\'B1":
    verilog1SingleApostropheB1Tok
  of "1\'BX":
    verilog1SingleApostropheBXTok
  of "1\'Bx":
    verilog1SingleApostropheBxTok
  of "1\'b0":
    verilog1SingleApostropheb0Tok
  ...
  of "@@":
    verilogDoubleAtTok
  of "PATHPULSE$=":
    verilogPATHPULSESingleDollarSingleEqualTok
  of "[":
    verilogSingleLBrackTok
  of "[*":
    verilogSingleLBrackSingleAsteriskTok
  ...
  of "[=":
    verilogSingleLBrackSingleEqualTok
  of "[â€“>":
    verilogSingleLBrackSingleâSingle€Single“SingleGreaterThanTok
  of "\\":
    verilogSingleBackslashTok
  of "]":
    verilogSingleRBrackTok
  ...
  of "void":
    verilogVoidTok
  of "void\'":
    verilogVoidSingleApostropheTok
  of "wait":
    verilogWaitTok
  of "wait_order":
    verilogWaitOrderTok
  ...
  of "~|":
    verilogSingleTildeSinglePipeTok
  of "â€“":
    verilogSingleâSingle€Single“Tok
  of "â€“>":
    verilogSingleâSingle€Single“SingleGreaterThanTok
  of "â€“â€“":
    verilogSingleâSingle€Single“SingleâSingle€Single“Tok
  of "ERROR":
    verilogSyntaxError
  else:
...
proc slice*(node: VerilogNode): Slice[int] =
